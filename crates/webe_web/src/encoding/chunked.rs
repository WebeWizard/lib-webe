...
impl<B: AsyncBufRead + Unpin> AsyncRead for ChunkedDecoder<B> {
    fn poll_read(
        ...
        let limit = std::cmp::min(*this.cur_chunk_size - *this.cur_chunk_pos, buf.remaining());
...
async fn chunked_encoder() {
    ...
    chunked_writer.write_all(b"Wiki").unwrap();
    chunked_writer.write_all(b"pedia").unwrap();
    chunked_writer.write_all(b" in\r\n\r\nchunks.").unwrap();
    let final_chunk_len = chunked_writer.write(b"").unwrap(); // ending chunk
    assert_eq!(final_chunk_len, 0);
    assert_eq!(
