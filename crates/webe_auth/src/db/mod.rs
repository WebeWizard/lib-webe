...
impl AccountApi for DBManager {
                ...
                match err {
                    DieselError::DatabaseError(dberr, _) => match dberr {
                        DatabaseErrorKind::UniqueViolation => {
                            return Err(DBApiError::AlreadyExists);
                        }
    ...
    fn find(&self, acc_id: &u64) -> Result<Account, DBApiError> {
        let mut conn = self.get()?;
        let account = webe_accounts.find(acc_id).first(&mut conn)?;
        return Ok(account);
    }

    fn find_by_email(&self, account_email: &String) -> Result<Account, DBApiError> {
        let mut conn = self.get()?;
        let account = webe_accounts
    ...
    fn delete(&self, account: Account) -> Result<(), DBApiError> {
        let mut conn = self.get()?;
        let result = diesel::delete(&account).execute(&mut conn)?;
        if result == 0 {
            return Err(DBApiError::NotFound);
        };
        return Ok(());
...
impl SessionApi for DBManager {
    ...
    fn find(&self, session_token: &str) -> Result<Session, DBApiError> {
        let mut conn = self.get()?;
        let session = webe_sessions.find(session_token).first(&mut conn)?;
        return Ok(session);
    }
    ...
    fn delete(&self, session_token: &str) -> Result<(), DBApiError> {
        let mut conn = self.get()?;
        let result =
            diesel::delete(webe_sessions.filter(token.eq(session_token))).execute(&mut conn)?;
        if result == 1 {
            return Ok(());
        } else {
            return Err(DBApiError::NotFound);
