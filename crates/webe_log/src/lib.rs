...
pub struct WebeLogger {
    ...
    _monitor: JoinHandle<()>, // a separate thread used to push logging status messages to console
    pub mon_sender: mpsc::Sender<String>,
}

impl Default for WebeLogger {
    fn default() -> Self {
        Self::new()
    }
}
...
impl WebeLogger {
    pub fn new() -> WebeLogger {
        ...
        let monitor = thread::spawn(move || while let Ok(_status_msg) = mon_recv.recv() {});
        WebeLogger {
            sinks: Vec::new(),
            _monitor: monitor,
            mon_sender: mon_send,
...
pub trait Sink {
    ...
    fn write(&mut self, _level: &LogLevel, _msg: &str) {}
}
