...
pub struct Server {
  pub ip: Ipv4Addr,
  pub port: u16,
  pub routes: Arc<RouteMap>, // server must not live longer than routes
...
impl Server {
  ...
  pub fn start(&self) -> Result<(), ServerError> {
    ...
    match thread::scope(|s| {
      for stream in self.listener.incoming() {
        match stream {
          Ok(stream) => {
            s.spawn(move |_| Server::process_stream(&stream, &self.routes));
          }
  ...
  pub fn process_stream<'s>(stream: &'s TcpStream, routes: &Arc<RouteMap>) -> Result<(), ServerError> {
                ...
                Some(route) => {
                  match routes.get(route) {
                    Some(responder) => {
                      ...
                      // TODO: ****STOPPED HERE******
                      // need to make sure that responder.build_response takes body_reader as parameter
                      // OR , figure out how to make the body_reader part of the request object again
