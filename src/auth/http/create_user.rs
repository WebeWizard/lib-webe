...
impl<'w> Responder for CreateUserResponder<'w> {
  fn validate(&self, request: &Request, _params: &HashMap<String, String>) -> Result<u16, u16> {
  ...
  fn build_response(
    ...
    let static_responder: StaticResponder = if validation_code != 200 {
    ...
    return static_responder.build_response(request, params, 1);
  }
}
