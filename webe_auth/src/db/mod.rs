...
impl AccountApi for DBManager {
  ...
  fn find(&self, account_id: &u64) -> Result<Account, DBApiError> {
    let conn = self.get()?;
    let account = webe_accounts.find(account_id).first(&conn)?;
    return Ok(account);
  }
  ...
  fn verify(&self, account_id: &u64, code: &String) -> Result<(), DBApiError> {
    let conn = self.get()?;
  ...
  fn reset_verification(
    &self,
    account_id: &u64,
    new_verify: (String, u32),
  ) -> Result<(), DBApiError> {
    let conn = self.get()?;
    let result = diesel::update(webe_accounts.find(account_id))
      .set((
...
trait SessionApi {
  ...
  fn delete(&self, token: &String) -> Result<(), DBApiError>;

  fn revoke_all(&self, account_id: &u64) -> Result<(), DBApiError>;
}
