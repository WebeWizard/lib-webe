...
{
  match webe_users
    .filter(account_id.eq(acc_id).and(name.eq(user_name)))
    .first(connection)
  {
    Ok(account) => return Ok(account),
    Err(err) => return Err(UserApiError::DBError(err)),
...
where
  T: diesel::Connection<Backend = diesel::mysql::Mysql>, // TODO: make this backend generic
{
  match diesel::delete(webe_users::table.find(user_id)).execute(connection) {
    Ok(_) => return Ok(()),
    Err(err) => return Err(UserApiError::DBError(err)),
  }
}
